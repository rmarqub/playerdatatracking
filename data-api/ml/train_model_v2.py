...
Versión v2 — objetivo: reducir overfitting y mejorar calidad probabilística.

Mantiene el output principal del script original:
...
Cambios clave:
  - Excluye season como feature predictiva por defecto.
  - Permite excluir league_id crudo si domina demasiado (--drop-league-id).
  - Limpieza/pruning de features por nulos, constantes y colinealidad simple.
  - Selección temporal de features usando SOLO train y una validación interna.
  - Regularización más fuerte.
  - Corrección de prior cuando se usa class_weight.
  - Temperature scaling sobre validación interna para probabilidades más conservadoras.

Uso:
    python train_model_v2.py --input training_data.parquet --test-seasons 2025
    python train_model_v2.py --input training_data.parquet --test-seasons 2025 --max-features 70 --drop-league-id
"""
...
NON_FEATURES = {
    ...
    # Evita que el modelo memorice una temporada concreta. La temporalidad ya entra en rolling/season form.
    "season",
}

# league_id queda como categórica opcional; se puede eliminar con --drop-league-id.
...
class CalibratedLGBM:
    """Wrapper ligero y picklable para corregir class_weight y aplicar temperature scaling."""
    ...
    def _prior_correct_multiclass(self, probs: np.ndarray) -> np.ndarray:
        ...
        corrected = probs / np.clip(weights, 1e-12, None)
...
def select_top_features(train: pd.DataFrame, features: list[str], target: str, max_features: int,
                        task: str) -> list[str]:
    ...
    selected = imp[imp > 0].head(max_features).index.tolist()
    if not selected:
...
def _find_best_temperature_multiclass(model, val: pd.DataFrame, features: list[str], class_weight: dict) -> float:
    ...
    scores = []
    for t in candidates:
        p = _softmax_from_probs(corrected, t)
        scores.append((rps_score(y, p), t))
    return min(scores)[1]
...
def _find_best_temperature_binary(model, val: pd.DataFrame, features: list[str], target: str,
    ...
    scores = []
    for t in candidates:
        pt = _sigmoid_from_probs(p, t)
        scores.append((log_loss(y, np.column_stack([1 - pt, pt])), t))
    return min(scores)[1]


def train_1x2(train: pd.DataFrame, features: list[str]) -> CalibratedLGBM:
    # Peso suave para empate: evita colapso a H/A sin inflar X al 32% como ocurría con peso alto.
    cw = {0: 1.0, 1: 1.12, 2: 1.0}
...
def train_binary(train: pd.DataFrame, features: list[str], target: str) -> CalibratedLGBM:
    # scale_pos_weight suave según frecuencia en train. Corrige posterior después en wrapper.
...
def evaluate_binary(model: CalibratedLGBM, data: pd.DataFrame, features: list[str], target: str,
                    label: str, split_label: str = "Test") -> float:
    X = data[features]
...
def parse_args():
    ...
    p.add_argument("--max-features", type=int, default=80,
                   help="Máximo de features tras selección temporal. 0 desactiva selección.")
    p.add_argument("--drop-league-id", action="store_true",
                   help="Elimina league_id crudo para evitar memorización por liga.")
    p.add_argument("--max-null", type=float, default=0.45,
                   help="Descarta features con más de este porcentaje de nulos en train.")
    return p.parse_args()


def _prepare_features(df: pd.DataFrame, train: pd.DataFrame, base_features: list[str], args, target: str, task: str):
...
def main():
    ...
    meta_base = {
        ...
        "config_name": "v2_regularized_calibrated_feature_selected",
    }
