...
def compute_league_rates(fixtures: pd.DataFrame) -> pd.DataFrame:
    ...
    Para cada partido calcula, con todos los partidos PREVIOS de esa liga:
      ...
      - league_draw_rate     : tasa histórica de empates en esa liga
      - league_away_win_rate : tasa histórica de victorias visitantes en esa liga
      - league_over25_rate   : tasa histórica de over 2.5 en esa liga
      - league_btts_rate     : tasa histórica de BTTS en esa liga
...
def assemble_dataset(
    ...
    # Match balance index — señal conservadora para empates: combina igualdad de PPG y XG.
    if "ppg_balance" in df.columns and "xg_balance" in df.columns:
        df["match_balance_index"] = df[["ppg_balance", "xg_balance"]].mean(axis=1)

    # Diferencia entre probabilidad base de liga y tendencia de los equipos.
...
def print_diagnostics(fixtures: pd.DataFrame, dataset: pd.DataFrame, n: int,
    ...
    checks = {
        ...
        "league_away_win_rate":                           "League away win rate",
