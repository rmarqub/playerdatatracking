...
public class LoginController {
	...
	public LoginController(RsaKeyProvider keys, UserService users, CryptoRSAService crypto) { 
		this.keys = keys;
		this.users = users;
        this.crypto = crypto;
	}
    ...
    public ResponseEntity<?> login(@RequestBody Map<String, String> body, HttpServletRequest request) {
        try {
            ...
            String cipherB64 = body.get("pwd"); // contraseña cifrada en Base64 desde el front
    ...
    public ResponseEntity<?> register(@RequestBody Map<String, String> body) {
      try {
        ...
        users.register(username, rawPassword);

        //NO autologuear tras registrar
        return ResponseEntity.status(HttpStatus.CREATED).body(Map.of("ok", true));

        //Autologuear automáticamente
        // var claims = Map.of("id", newUser.getId(), "username", newUser.getUsername(), "roles", List.of("USER"));
        // (reutiliza el mismo bloque de login para crear sesión + SecurityContext)
